...
| **App code** | [`skill-reputation/`](./skill-reputation/) — all commands run from this folder |
| **Docs UI** | [`skill-reputation/demos/demo-ui/`](./skill-reputation/demos/demo-ui/) (optional Vercel deploy) |
| **Demo video** | Script: [`skill-reputation/DEMO.md`](./skill-reputation/DEMO.md) (~3 min) |
...
Paths below are under `skill-reputation/`:

| What to check | Where |
|---------------|--------|
| Best backtest | **Regime / BTC · Sharpe 2.17** → `skill-reputation/backtest_results/regime_BTC.json` |
| CMC skill format | `skill-reputation/skills/cmc-strategy-{momentum,sentiment,regime}/SKILL.md` |
| Skill export zip | `skill-reputation/examples/cmc-strategy-skills.zip` after `npm run export:skills` |
| Replay report | `skill-reputation/replay/output/replay_report.html` |
| MCP (local) | `npm run mcp:forge` + [`forge-mcp-config.json`](./skill-reputation/forge-mcp-config.json) |
| Checklist | [`SUBMISSION.md`](./skill-reputation/SUBMISSION.md) · [`DEMO.md`](./skill-reputation/DEMO.md) |

**No Render or public server required** — judges clone GitHub and run locally. Optional hosted agent server: [`bnbagent/RENDER_DEPLOY.md`](./skill-reputation/bnbagent/RENDER_DEPLOY.md) (later).
...
Reproduce locally from `skill-reputation/` (requires `.env` with wallet — **never commit**):
...
|------|----------------|-------|
| 1 | Live CoinMarketCap signals | `skill-reputation/src/cmcDataClient.js`, `cmcSignals.js` |
| 2 | Backtest with honest metrics | `skill-reputation/backtest_results/*.json` (9 files) |
| 3 | Official CMC skill format | `skill-reputation/skills/cmc-strategy-*/SKILL.md` |
...
Reproduce from `skill-reputation/`: `npm run strategy:all` with `CMC_API_KEY` and `CMC_USE_MOCK=0`.
...
Config: [`forge-mcp-config.json`](./skill-reputation/forge-mcp-config.json) · BNB MCP: [`bnb-mcp-config.json`](./skill-reputation/bnb-mcp-config.json)

**Cursor MCP** (Settings → MCP — set `cwd` to your `skill-reputation` clone path):
...
```json
{
  "mcpServers": {
    "forge-skills": {
      ...
      "cwd": "/absolute/path/to/openskill/skill-reputation"
    }
  }
}
```

Interactive docs + Ask Docs → [`demos/demo-ui/`](./skill-reputation/demos/demo-ui/)
...
└── skill-reputation/          ← npm / Python app root
...
Copy [`skill-reputation/.env.example`](./skill-reputation/.env.example) → `skill-reputation/.env` (**never commit** `.env` or `bnbagent/.env.agent`).
...
| Resource | Path |
|----------|------|
| Submission checklist | [SUBMISSION.md](./skill-reputation/SUBMISSION.md) |
| Demo video script | [DEMO.md](./skill-reputation/DEMO.md) |
| Optional Render deploy | [bnbagent/RENDER_DEPLOY.md](./skill-reputation/bnbagent/RENDER_DEPLOY.md) |
